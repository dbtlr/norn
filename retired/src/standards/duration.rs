...
//!
//! Kept free of intra-crate dependencies: `build.rs` includes this file via
//! `#[path]` (as `mod standards`) so `src/cli.rs` can reference
//! `crate::standards::parse_duration` from both compilations — the same
//! include trick build.rs already uses for `cli.rs` itself.
