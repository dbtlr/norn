//! Unit tests for the vault-env seam (cold/warm behavior, generations,
//! freshness pipeline, and the request-scope boundary).
...
// Test-only cross-submodule references that the production hub imports in
// `mod.rs` do not need (nothing outside tests calls these across a submodule
// boundary), so they are pulled in directly here rather than widening the
// hub's own import list.
use super::ensure::map_open_outcome;
use super::error::is_sqlite_corruption;
use super::refresh::{RefreshArrival, RefreshOutcome};
