...
- [ ] `src/set/`, `src/new/`, `src/edit/`, `src/move/`, `src/delete/`, `src/rewrite_wikilink/` → mutation verbs (partial: `src/set/` and `src/new/` ported in NRN-378 — the FIRST mutation-verb port, establishing the pattern the rest follow. `set` and `new` land as surface-neutral execute seams in `norn-core::mutate` (`mutate::set::execute` / `mutate::new::execute`, both `(cache, config, params, today, sink) -> MutationExecution<Report>`): synth the frontmatter-field ops / `create_document` op into a `MigrationPlan`, apply via `apply::apply_migration_plan` under `refuse_as_report` (a clean pre-write decline is a coded `outcome=refused` report, not a bare Err), and hand the owner `touched_paths` for the cache-increment commit. The wire vocabulary is `norn-wire::mutate` (`SetParams`/`SetReport`/`NewParams`/`NewReport` + `MutationOutcome`/`CodedError`/`MutationWarning`/`FrontmatterChange`/`FrontmatterCreated`), the frames `ClientFrame::{Set,New}` / `OwnerFrame::{Set,New}`, the owner serializes every write under a new in-process `mutation_lock` (the ADR 0013/0017 successor to the donor mutation flock) and drives the seam via `run_mutation` (baseline capture → serve_read → fire-and-degrade increment), the client speaks `session.set`/`session.new_document` (send-once, never-retry — no double-apply, ADR 0011), and the CLI is `commands::{set,new}` returning `Output::{Set,New}` rendered by the one `emit` layer (change-line primitive in `output::primitives::change_line`, the set change-lines + new key/value block). `edit/transform.rs`'s `SectionSpan` + `resolve_section` heading→span primitive already ported to `norn-frontmatter::section` in NRN-339; the section-edit TRANSFORM ENGINE (`edit/ops.rs` `EditOp` + `edit/transform.rs` `apply_edits`) ported in NRN-386 → `norn-core::edit` (the executor's compose path needs it); the edit-op grammar's VERB surface (`edit/route`/`report`/`sugar`/`synth`) and `edit`/`move`/`delete`/`rewrite_wikilink` verbs remain)
