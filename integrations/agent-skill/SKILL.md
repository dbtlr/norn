...
Stable finding codes (15): `read-failed`, `frontmatter-unclosed`, `frontmatter-parse-failed`, `frontmatter-json-conversion-failed`, `link-target-missing`, `link-anchor-missing`, `link-block-missing`, `link-ambiguous`, `frontmatter-required-field-missing`, `frontmatter-forbidden-field`, `field-type-invalid`, `frontmatter-exceeds-max-length`, `value-not-allowed`, `document-misrouted`, `frontmatter-reference-type`. See [validation.md](https://github.com/dbtlr/norn/tree/main/docs/validation.md) for severity and source per code. Renames are CHANGELOG breaking changes. Aliases (the `aliases` frontmatter field) are plain queryable metadata — they do not participate in link resolution; a dangling wikilink that uniquely matches one doc's alias is offered as a `repair` rewrite-to-stem hint.
