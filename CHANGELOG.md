...
- **An ambiguous-key document is refused whole, never partially edited.** The span locator confirmed each serde key it could uniquely place and dropped the rest, but still emitted spans for the placed keys. A key the scanner mis-decodes (a double-quoted escape such as `"\x61"`, which serde reads as `a` but the scanner reads as `x61`) matched no candidate line, so its bytes were absorbed into a neighboring field's line span — a `remove`/`set` of that unrelated field silently deleted the mis-decoded real key. When any serde key cannot be located to exactly one candidate line (zero, or more than one), the locator now refuses spans for the entire document; `set`/`remove` report each field not-editable and write nothing, so an ambiguous-key document is declined cleanly rather than corrupted. (Full structural span parsing is tracked as NRN-140.) (NRN-141)
- **A collection `set` preserves a trailing same-line comment.** A list/flow `set` rewrites the whole field line, which dropped a trailing comment: `set tags=[…]` on `tags: [a, b]  # note` lost the `# note`. A single-line collection `set` now recaptures the trailing comment (with its leading whitespace) and re-appends it, so `tags: [a, b]  # note` becomes `tags: [x]  # note`. A `#` inside a quoted scalar (`tags: ["a#b"]`) is not treated as a comment. Multi-line flow values are out of scope — their comment is still dropped. (NRN-141)
