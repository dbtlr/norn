...
- **Derived `document_fields` cache index table (schema version 2 → 3).** The cache gains an internal EAV table shredded from the declared/bounded frontmatter field set — one row per (document, indexed field), typed and canonicalized to match query semantics, with a sentinel row for fields absent or null. It's kept current by every rebuild/incremental index run and by a self-healing re-shred (regenerated from the cached frontmatter, no filesystem re-parse) whenever the resolved index set changes. The schema bump means every existing cache silently rebuilds once on first use after upgrading, exactly like any other schema-version bump.
- **`find`/`count` route qualifying queries through the `document_fields` index.** `--eq`, `--not-eq`, `--in`, `--not-in`, `--has`, and `--missing` on a declared-`indexed` field now answer from `document_fields` instead of scanning `frontmatter_json` via `json_extract` — same results, faster on large vaults (`norn count` shares the same query builder as `find`, so it routes identically). Routing is all-or-nothing per query: it only engages when every predicate present is one of the six kinds above with string-shaped values and every field they touch is indexed; a query mixing in `--starts-with`/`--ends-with`/`--contains`, a date predicate (`--before`/`--after`/`--on`), or a non-string `--eq`/`--in` value falls back to the unchanged scan path for the whole query (a provable-equivalence gap in those forms, not a defect — see `src/cache/scan_semantics_probe.rs`). Falling back because a referenced field simply isn't indexed additionally warns once on stderr, but only past 1,000 documents: `scanned N documents on unindexed field(s) 'x', 'y'; declare indexed: true (or a bounded type) to accelerate`. No output, exit-code, or ordering changes on any query — routing is purely an internal query-plan choice.
