...
- **Anchored string field operators — `--starts-with` / `--ends-with` / `--contains` on `find` and `count`.** Frontmatter fields gain prefix/suffix/substring predicates (`FIELD:VALUE` tokens, repeatable, AND-composed like `--eq`), enabling namespace enumeration over tag conventions — `norn find --starts-with tags:release:` returns every document carrying a `release:*` tag. The operators are case-sensitive, treat `%`/`_` and every other character literally (no wildcards or regex), apply to scalar fields and array elements alike, and collapse `[[wikilink]]` brackets on both sides (`--starts-with depends_on:NRN-` matches a stored `"[[NRN-123]]"`). Non-string stored values compare by their text rendering. The `vault.find` and `vault.count` MCP tools gain the matching `starts_with`/`ends_with`/`contains` params. `--contains` is frontmatter-scoped; body substring search remains `--text`.
