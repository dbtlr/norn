...
- **`vault.set`'s schema-validation refusals now carry a stable machine-branchable `code`, closing the gap NRN-220 deliberately deferred (NRN-221).** A `norn set` / `vault.set` call that hits a schema or argument refusal — an out-of-schema field type, a value outside `allowed_values`, a `--remove` of a `required_frontmatter` field, a malformed `KEY=VALUE`, a `--push` on a scalar, an unresolved or ambiguous `DOC` target, a cross-class key conflict, or a non-mapping/unparseable frontmatter — previously propagated as plain prose over a bare `Err`, landing in the documented "uncoded" bucket on both surfaces: MCP `vault.set` returned an opaque JSON-RPC error with no `report.error.code`, and CLI `--format json` laundered it into the generic `internal-error` code. Both now surface one of eleven new kebab codes — `field-type-invalid`, `value-too-long`, `value-not-allowed`, `field-json-invalid`, `required-field-removed`, `target-not-found`, `target-ambiguous`, `assignment-malformed`, `field-conflict`, `push-on-scalar`, `frontmatter-not-mapping` (plus the existing `frontmatter-parse-failed` for a target document with unparseable on-disk frontmatter) — documented in `docs/errors.md`. MCP `vault.set` now returns the SAME structured `isError: true` + `report.error.code` shape NRN-220 gave `edit`/`new`, instead of a bare `Err`. Human-facing prose (CLI TTY/stderr text, and every error's `Display`) is byte-identical to before; only the machine-readable `code` is new. No other command's refusal codes change.
