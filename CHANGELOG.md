...
- **`norn get --section "Heading"` reads named sections of a document's body**, closing the read/write asymmetry with `edit`'s section ops: it resolves each requested heading with the exact same boundary semantics `edit --append-to-section`/`--replace-section` use (heading line through the next same-or-higher heading, or EOF), so a section read mirrors a section write. Accepts a comma-separated list (`--section "Task Description","Annotations"`) and is orthogonal to `--col`/`--all-cols` — combine freely. `--format json`/`jsonl` add a `sections` object keyed by the requested heading text; `records` prints each requested section as its own block. A heading missing or ambiguous in a given document warns on stderr and is omitted from that document's `sections` without affecting sibling headings or other targets; if none of the requested headings resolve for a document, that document counts toward `get`'s existing nonzero-exit contract (mirroring an unresolved target), though its record still returns. Ignored, with a warning, by `--format paths`/`markdown`, like `--col`. (NRN-102)
