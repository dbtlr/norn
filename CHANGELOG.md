...
- **Three MCP capability gaps that forced Mimir-style consumers into filesystem workarounds are closed, all additive on the wire.** (1) `vault.get` reaches full parity with `norn get`'s daily surface: it gains `sort`/`desc`/`limit`/`no_limit`/`starts_at` (the shared sort-and-paging knobs, same names and defaults as the CLI, applied in-memory over the resolved record set) and `all_cols` (load the full structured dump including `body`), plus `section` — a repeatable list of exact heading strings that returns the same `sections` object `norn get --format json` emits, byte-for-byte, reusing the CLI's v0.44 section-slice seam. Per the CLI contract, a heading missing or ambiguous in a document is warn-and-omitted for that document; when *none* of the requested headings resolve for a target, that target is reported in an additive `section_failures` list (its path plus the requested headings) — the structured MCP mapping of the CLI's nonzero-exit signal — rather than failing the whole call, so a batch keeps every good target's records (and the all-missing target's own record, with an empty `sections`) instead of losing them to one sibling's miss. `limit` and `no_limit` are mutually exclusive (setting both is a params error, mirroring the CLI's clap conflict); on the get path `limit` defaults to `None`, so `no_limit` alone is already the default (return every target). (The `col` semantics divergence — the CLI narrows the projection, MCP only opts extra facets in — is deliberately left unchanged, deferred to a later break window.) (NRN-173) (2) `vault.apply_plan` gains `parents` (default `false`), auto-creating missing parent directories for `create_document` ops that proceed (`mkdir -p` style) — the applier already supported it end-to-end; it just wasn't exposed. `norn migrate` gains the matching `-p`/`--parents` flag in lockstep, so the CLI/MCP pair acquire the capability together. This retires a consumer's out-of-band `mkdir` before writing. (NRN-174) (3) `ApplyReportOp` (every plan-apply report op, over both `norn migrate --format json` and `vault.apply_plan`) gains two optional structured fields `path` and `stem` — the apply-time-resolved target (e.g. a `{{seq}}`-expanded `create_document` destination), so a consumer correlates the created/moved/edited document by `op_id` instead of regexing the free-prose `summary`. Both are populated where a single natural target exists (create destinations, move destinations, body/section edit targets) and omitted otherwise, keeping the report byte-identical for ops with no natural path. (NRN-175)
...
- **The bundled agent skill's apply-model table no longer tells an unattended agent that `migrate`/`rewrite-wikilink` apply without `--yes`.** Both commands implicitly dry-run in a non-TTY invocation without `--yes` — the table previously listed them as applying, which would read as a silent no-op success to an agent that trusted it. The one real divergence is documented instead of papered over: both commands treat `--format json` as consent to apply, unlike the rest of the mutation surface, where JSON output is still an implicit dry-run. The skill's `validate` guidance is also corrected: the exit code reflects whole-vault error-severity diagnostics regardless of `--code`/`--severity`/`--path` filters, and most finding codes default to `warning` severity, so gating a pipeline on exit code alone was never reliable — the skill now points agents at `--summary` totals or the findings themselves. `get --section` gains the missing note that a target hard-fails when *every* requested heading is missing, rather than always warn-and-omit. (NRN-194)
