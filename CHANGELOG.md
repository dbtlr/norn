...
- **`norn service` — a launchd supervisor over the warm `norn serve` daemon (macOS) (NRN-115).** `norn serve` is a plain foreground process; the new `norn service install | uninstall | start | stop | restart | status` verbs adopt it as a launchd user agent so it is always warm (`KeepAlive` + `RunAtLoad`) — the same install-once-stay-warm posture Mimir's service contract uses, with no CLI auto-spawn. `install` renders a plist at `~/Library/LaunchAgents/com.dbtlr.norn.serve.plist` (`ProgramArguments` = the resolved absolute binary + `serve`; stdout/stderr → `<XDG_CACHE_HOME>/norn/log/serve.log`) and bootstraps it idempotently (bootout-first, with a retry on launchd's async-teardown race); `uninstall` boots it out and removes the plist (config and logs kept; tolerant of not-installed); `start` reloads a stopped unit; `stop` is an honest bootout (a `KeepAlive` daemon would resurrect a merely-killed pid); `restart` is `kickstart -k`. Lifecycle verbs act only on an installed unit and exit nonzero when there was nothing to act on, so a `norn service restart && …` deploy chain never proceeds on a no-op. `status` combines `launchctl print` (loaded / running / pid) with a live control-ping over the daemon's Unix socket, rendering the running vs on-disk build version with a **restart-pending** flag (plus uptime and the plist / log / socket paths). Every verb takes `--format text|json`. On non-macOS hosts the verbs print a friendly fallback (run `norn serve` under your own supervisor; systemd support is planned) and exit nonzero.
