...
- **`norn edit` and MCP `vault.edit` accept an optional expected-hash compare-and-swap.** Pass `--expected-hash <HASH>` (CLI) or `expected_hash` (MCP) — the blake3 hex of the document's full content, as `norn get` reports — and the edit is refused (pre-flight, exit 2, nothing written) if the document has drifted from that hash. This lets a caller that read a document, decided on an edit, and wants to guard against a concurrent writer detect the drift instead of silently clobbering it. The flag is opt-in: omitted, `edit` behaves exactly as before (read-modify-write), preserving the one-shot common case. CLI and MCP are at parity (the check lives in the shared edit preflight).
