...
- **The MCP mutation/query tools reach parity with their CLI twins on four flags an agent previously could not use over MCP.** `vault.move` gains `force` (overwrite an existing destination) and `no_link_rewrite` (move the file but leave incoming backlinks pointing at the old target) — single-file moves only, matching `norn move --force`/`--no-link-rewrite` (NRN-180). `vault.set` gains `field` (a `KEY=VALUE` map that string-coerces against the schema, the coercing counterpart to the existing typed `set` JSON map) plus `push`/`pop` maps for list-typed fields, mirroring `norn set --field`/`--push`/`--pop` (NRN-181). `vault.validate` gains `summary: true`, returning the grouped finding-count rollup (by code/severity/rule/field/path-prefix) computed by the same code path as `norn validate --summary`, instead of the raw findings list (NRN-182). `vault.audit`'s `status` filter is now typed as the closed `applied`/`skipped`/`failed` enum the CLI enforces, so the published tool schema advertises the valid set and rejects a typo as a params error rather than silently matching nothing (NRN-184). (NRN-179)
