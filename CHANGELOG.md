...
- **Derived `document_fields` cache index table (schema version 2 → 3).** The cache gains an internal EAV table shredded from the declared/bounded frontmatter field set — one row per (document, indexed field), typed and canonicalized to match query semantics, with a sentinel row for fields absent or null. It's kept current by every rebuild/incremental index run and by a self-healing re-shred (regenerated from the cached frontmatter, no filesystem re-parse) whenever the resolved index set changes. Not read by any query path yet — this lands the writer only, ahead of the query router. The schema bump means every existing cache silently rebuilds once on first use after upgrading, exactly like any other schema-version bump.
