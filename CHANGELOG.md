...
- **Three MCP capability gaps that forced Mimir-style consumers into filesystem workarounds are closed, all additive on the wire.** (1) `vault.get` reaches full parity with `norn get`'s daily surface: it gains `sort`/`desc`/`limit`/`no_limit`/`starts_at` (the shared sort-and-paging knobs, same names and defaults as the CLI, applied in-memory over the resolved record set) and `all_cols` (load the full structured dump including `body`), plus `section` — a repeatable list of exact heading strings that returns the same `sections` object `norn get --format json` emits, byte-for-byte, reusing the CLI's v0.44 section-slice seam. Per the CLI contract, a heading missing or ambiguous in a document is warn-and-omitted for that document; when *none* of the requested headings resolve for a target, the call hard-fails (mirroring the CLI's nonzero-exit signal). (The `col` semantics divergence — the CLI narrows the projection, MCP only opts extra facets in — is deliberately left unchanged, deferred to a later break window.) (NRN-173) (2) `vault.apply_plan` gains `parents` (default `false`), auto-creating missing parent directories for `create_document` ops that proceed (`mkdir -p` style) — the applier already supported it end-to-end; it just wasn't exposed. `norn migrate` gains the matching `-p`/`--parents` flag in lockstep, so the CLI/MCP pair acquire the capability together. This retires a consumer's out-of-band `mkdir` before writing. (NRN-174) (3) `ApplyReportOp` (every plan-apply report op, over both `norn migrate --format json` and `vault.apply_plan`) gains two optional structured fields `path` and `stem` — the apply-time-resolved target (e.g. a `{{seq}}`-expanded `create_document` destination), so a consumer correlates the created/moved/edited document by `op_id` instead of regexing the free-prose `summary`. Both are populated where a single natural target exists (create destinations, move destinations, body/section edit targets) and omitted otherwise, keeping the report byte-identical for ops with no natural path. (NRN-175)
