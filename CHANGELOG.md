...
- **A vault mutation can no longer write outside the vault root.** `create`/`new`, `move`, `delete`, and `edit`/apply targets are now checked against the vault root before any byte is written: an absolute path, a `..` parent-traversal component, or a target reached through a directory symlinked out of the vault is refused as a preflight (the offending path is named; exit 2). Previously all four escapes wrote outside the vault — `norn new`'s lexical `..`/absolute check was the only guard, and a directory symlinked out of the vault bypassed it entirely. The vault is now treated as self-contained: outbound symlinks are unsupported. One shared containment gate backs the whole mutation stack and `norn new` (no parallel logic), and it canonicalizes the nearest existing ancestor of the target's parent, so `-p`/`--parents` creation of a not-yet-existing subtree cannot be used to sidestep it. (NRN-145)
