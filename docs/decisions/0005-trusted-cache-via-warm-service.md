...
description: "Architectural decision relocating norn's SQLite cache integrity verification from a per-invocation CLI check to a persistent warm norn-service process, with the CLI falling back to direct verification when no service is available. Amended 2026-07-17 (ADR 0017): the daemon becomes the sole access path; the direct fallback described here is historical."
---

# 0005 — Cache integrity is relocated to a warm service, never weakened

> **Amended by [0017](./0017-registered-vaults-summoned-owners.md) (2026-07-17):** every invocation now routes to a summoned owner and the Direct verified-open fallback is deleted — the direct-path behavior below is historical. See the dated amendments at the end.
