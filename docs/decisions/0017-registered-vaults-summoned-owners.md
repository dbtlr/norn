...
- **Writes.** Each owner holds its own single-writer lock, so the serialization boundary is per owner, not per vault. Cross-owner file writes degrade onto the foreign-writer path: the applier's fingerprint compare-and-set sees the other owner's bytes and refuses rather than clobbering.
- **`{{seq}}` allocation and multi-file plan atomicity are per owner.** Two owners can allocate the same `{{seq}}` id concurrently; the loser's create finds the destination occupied and refuses `destination-already-exists` (loud, never a silent overwrite without `--force`).
