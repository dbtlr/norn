...
- **The terminal publication is one non-preemptible transaction.** Its work scales with the number of affected document-owned rows plus the vault's complete links set. Representative release-mode dogfood stayed below the 50 ms chunk target for a normal one-document mutation, but a synthetic all-document publication exceeded it. This is the deliberate atomicity tradeoff; making the final switch O(1) would require a persistent double-buffered cache layout.
