...
A `target` may include a `{{seq}}` token to allocate the next integer id at creation time, e.g. `target: "tasks/MMR-{{seq}}.md"`. The id is `max + 1` over the existing files sharing the resolved prefix, so it is **scoped per prefix** — `MMR-{{seq}}` counts only `MMR-*` files (independent of `NRN-*`), and the first id is `1`. Allocation happens at apply time under the mutation lock held by the daemon serving the vault, so two concurrent creations through one daemon get distinct, sequential ids with no collision.

That lock is per *daemon*, and a daemon is addressed by vault root + binary build + config content — so during a transition window (you rebuilt norn, or you edited `.norn/config.yaml` and the previous daemon has not yet idled out) two daemons can briefly serve one vault and allocate the same id. The outcome is loud, never silent: the second creation finds the file already there and refuses `destination-already-exists` (only `--force` overwrites). Rerun it and it takes the next id.
