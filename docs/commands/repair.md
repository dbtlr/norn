...
Two built-in rewrites need no configured rule. A `link-target-missing` finding whose target uniquely matches exactly one document's `aliases` entry, and whose candidate stem is itself unique across the vault, plans a deterministic `rewrite_link` to that document's canonical stem (`built-in:alias-stem`) — the migration path for links that relied on the retired alias resolution (aliases are now plain queryable frontmatter, not a resolution target). Failing that, a close slug-normalized stem match plans a `built-in:closest-match-stem` rewrite. A target that matches an alias claimed by two or more documents, or whose candidate stem is shared by another document, gets no hint and stays dangling — a rewrite to a non-unique stem would trade a dangling link for an ambiguous one.
