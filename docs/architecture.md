...
**A boundary carries facts — severity and code enums — and every outcome is decided from them, never from a rendered sentence.** No outcome bit may be derived from message text: the code decides the outcome, the text only explains it. Message text does cross a boundary, as the `message` member of a typed envelope, and where one condition can arise under several verbs its wording is CANONICAL and built once (a failed target resolution is spelled by `norn-core`'s resolver, not by each verb) — so a message that crosses is a shared fact, not a per-surface rendering. What each client surface still owns is presentation around that fact: its prefix vocabulary, framing, and any surface-specific detail it composes from the typed members. The stderr prefix vocabulary is a closed set defined once in the display layer, and `format!("{:?}")` never appears in display code nor in a verb's message construction — a candidate path list reaches a reader as a joined list, never as a `Debug`-rendered vector (ADR [0020](./decisions/0020-posix-by-default.md), [0021](./decisions/0021-verbs-return-one-layer-renders.md)).
