//! The section-edit op set — a content-anchored tagged union, deserialized from
//! the `edits` array a mutation plan's section-edit ops carry. Internally tagged
//! on `op`; `deny_unknown_fields` is intentionally omitted (serde forbids it on
...
/// `Serialize` is derived so a plan can round-trip its section-edit ops through
/// JSON losslessly (the exact inverse of `Deserialize`).
