...
//! (resolved), unresolved links, and incoming links (other documents that link
//! *to* this one). Used by `show`.

use crate::domain::{Heading, Link, LinkStatus};
...
/// A back-link: another document's path plus the `Link` record that points at
...
/// pointing at the current document.
fn load_incoming(
    ...
    fn document_with_connections_uses_index_for_incoming_links() {
        ...
        for row in &plan_rows {
            assert!(
                ...
                "incoming-link query plan should not be a full scan: {row}"
            );
        }
    }
}
