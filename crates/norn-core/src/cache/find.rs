//! `find` cache-side types and entry point.
//!
//! `FindQuery` wraps [`DocumentQuery`](crate::query::DocumentQuery) (the
//! predicate set) with paging, sort, and limit. `FindResult` returns matches
//! plus the total count (before limit/offset) so callers can emit accurate
//! truncation signals.

use crate::domain::DocumentSummary;
use crate::query::DocumentQuery;

/// Top-level query for [`Cache::find_documents`](crate::cache::Cache::find_documents).
...
pub struct SortClause {
    ...
    /// corresponding columns); everything else is a frontmatter key resolved via
...
/// Result of `Cache::find_documents`.
#[derive(Debug, Clone)]
...
impl crate::cache::Cache {
    /// SQL-direct find with ORDER BY / LIMIT / OFFSET / COUNT. Path globs are
    /// applied as a Rust post-pass after SQL narrowing.
        ...
        let sql = if needs_post_pass {
            format!(
                "SELECT path, stem, hash, frontmatter_json, body_text \
                 FROM documents{where_sql} ORDER BY {order_by_sql}, path ASC"
            )
        } else {
            format!(
                "SELECT path, stem, hash, frontmatter_json, body_text \
                 FROM documents{where_sql} ORDER BY {order_by_sql}, path ASC LIMIT ? OFFSET ?"
            )
        };
...
/// Resolve a `SortClause` into a SQL ORDER BY fragment + any binds. Returns
/// `"path ASC"` when no sort is specified (the default stable order).
fn sort_clause_sql(sort: Option<&SortClause>) -> (String, Vec<rusqlite::types::Value>) {
    use crate::cache::query_documents::json_path_for;
    use rusqlite::types::Value as SqlValue;
...
mod tests {
    ...
    use crate::cache::{Cache, FindQuery, FindResult, SortClause, SortDirection};
    use crate::query::DocumentQuery;
    ...
    fn synth_paged_vault() -> (TempDir, Utf8PathBuf) {
        ...
        for i in 1..=25 {
            ...
            std::fs::write(
                root.join(format!("doc-{i:02}.md")).as_std_path(),
                format!("---\ntype: note\npriority: {priority}\n---\nbody-{i}\n"),
            )
            .unwrap();
        }
    ...
    fn find_documents_query_plan_is_single_select() {
        ...
        assert!(
            ...
            "find_documents plan contains a subquery: {rows:?}"
        );
        assert!(
            rows.iter().any(|r| r.contains("documents")),
            "plan does not mention documents table: {rows:?}"
        );
    }
}
