...
//! `find --help`. Aggregates top-level frontmatter field statistics and counts
//! documents matching a conjunction of predicates.
...
mod tests {
    ...
    fn empty_cache_returns_no_stats() {
        ...
        assert!(field_statistics(&cache).unwrap().is_empty());
    }
    ...
    fn count_matching_single_predicate() {
        ...
        insert_doc(&cache, "b.md", r#"{"type":"note"}"#);
        insert_doc(&cache, "c.md", r#"{"type":"task"}"#);
        assert_eq!(count_matching(&cache, &[("type", "note")]).unwrap(), 2);
    }
    ...
    fn count_matching_two_predicates_intersect() {
        ...
        assert_eq!(
            count_matching(&cache, &[("type", "note"), ("workspace", "norn")]).unwrap(),
            1
        );
    }
