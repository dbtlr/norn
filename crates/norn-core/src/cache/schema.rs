//! Cache schema DDL (ADR 0003 relational engine, ADR 0004 EAV frontmatter index).
...
-- Derived Wave-2 frontmatter index (EAV, ADR 0004). One row per (doc, declared
-- field) pair for every field in the resolved index set. `value` deliberately
-- has no type affinity: it stores typed scalars (INTEGER/REAL/TEXT), SQL NULL
-- (an empty-array field), or the `x'00'` BLOB sentinel (field absent/null).
...
pub(crate) fn apply_schema(conn: &Connection) -> Result<(), CacheError> {
