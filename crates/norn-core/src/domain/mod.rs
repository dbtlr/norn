//! The serializable graph types at the center of the crate.
//!
//! `Document`, `Link`, `GraphIndex`, and the `DocumentSummary` projection every
//! other module speaks in. Pure data with no I/O, so it is the shared vocabulary
//! rather than a dependency on any surface. `display` renders them for output.
//!
//! # Ported seams (ADR 0018)
//!
//! The text-layer types this model embeds — [`Diagnostic`] / [`Severity`],
//! [`Heading`], and [`SourceSpan`] — now live in `norn-frontmatter` and are
//! re-exported here so the domain vocabulary is unbroken. Do NOT redefine them.
//!
//! The [`Link`] value types ([`Link`], [`LinkKind`], [`LinkStatus`],
//! [`UnresolvedReason`], [`LinkSourceArea`], [`LinkSourceContext`]) are ported
//! here because they are core serializable domain types that [`Document`]
//! embeds. Link *resolution* (matching a target to a document, the graph build,
//! `resolve.rs`) is a separate port and is deliberately absent — a `Link` here
//! is a parsed, possibly-unresolved value, not proof that resolution ran.
...
pub use norn_frontmatter::heading::Heading;
pub use norn_frontmatter::{Diagnostic, Severity, SourceSpan};
...
/// command except document inspection, which needs the joined data and uses
/// `Document` directly.
