//! Resolved index-field set for the derived frontmatter index (the EAV
//! `document_fields` narrow table, ADR 0004). Pure config→policy function: no
...
/// Resolve the set of frontmatter fields to index, plus a stable hash of that
/// set (a lowercase-hex digest over the sorted, newline-joined field names).
///
...
/// constraint, or a bounded `field_types` type (`date`, `datetime`, `wikilink`,
/// `wikilink_or_list`, `string`, `list_of_strings`). `text` and undeclared
/// fields never auto-qualify.
pub fn resolved_index_set(cfg: &VaultConfig) -> (BTreeSet<String>, String) {
...
/// A stable, order-independent lowercase-hex digest of the field set: blake3 of
/// the sorted, newline-joined field names. Internal generation-identity only.
