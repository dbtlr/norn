...
/// The triage-filter inputs, one borrowed `&[String]` per flag. The CLI/owner
/// builds this from the wire `ValidateParams`.
#[derive(Debug)]
...
mod glob_match_tests {
    ...
    fn glob_star_matches_link_family() {
        ...
        let out = filter_findings(findings, &opts(&codes)).unwrap();
        assert_eq!(out.len(), 4);
    ...
    fn invalid_glob_returns_arg_error() {
        ...
        let result = filter_findings(vec![], &opts(&codes));
