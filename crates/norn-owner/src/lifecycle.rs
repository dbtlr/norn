...
/// open `File` — the caller MUST hold it for the process lifetime (dropping it
/// releases the lock). On contention (another owner already serves this
/// vault+build) returns an "already running" error carrying the incumbent pid;
/// the caller exits gracefully without binding.
pub fn acquire_owner_lock(lock_path: &Utf8Path, version: &str) -> anyhow::Result<std::fs::File> {
    use fs2::FileExt;
    ...
    match file.try_lock_exclusive() {
        ...
        // lock with `ErrorKind::WouldBlock`; report the incumbent pid if we can.
