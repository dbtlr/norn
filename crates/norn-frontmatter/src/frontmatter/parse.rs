...
/// The result of extracting a frontmatter block from a document.
///
/// `value` is the parsed frontmatter (`None` on a malformed or absent block).
/// `range` is the byte range of the YAML content between the delimiters — the
/// span `top_level_property_spans` scans; it is `None` when there is no block or
/// the block is unclosed. `body` is everything after the closing delimiter, and
/// `body_start` is its byte offset in the original document.
...
pub fn parse(content: &str) -> ParsedFrontmatter<'_> {
...
/// The lower-level extraction primitive, threading a caller-owned diagnostics
/// buffer so re-parses (e.g. the minimal-edit post-image gate) can reuse one
/// accumulator. Returns `(value, yaml_range, body, body_start)`.
...
mod tests {
    ...
    #[test]
    fn parse_wrapper_exposes_the_same_extraction() {
        let parsed = parse("---\ntitle: hello\n---\n# body\n");
        assert!(parsed.value.is_some());
        assert_eq!(parsed.range, Some(4..17));
