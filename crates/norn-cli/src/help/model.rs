...
//!
//! The `HelpExtras::live_examples_fn` hook is not wired: live examples would be
//! generated by opening the vault cache and running a query. `--help` opens a
//! freshly-created (empty) cache, so the generator would find zero documents
//! and emit nothing anyway; the hook is future work (returns when the query
//! core lands). `HelpModel::live_examples` stays and is always empty.
