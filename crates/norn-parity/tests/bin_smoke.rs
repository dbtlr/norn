...
fn self_check_end_to_end_is_all_match_exit_0() {
    ...
    // NRN-384 adds three more `mcp` tools/call cases (get-missing, count, validate)
    // atop the two existing handshake/get cases, taking the total to 96; self-check
    // ignores `ported`, so all Match (oracle vs. itself).
    assert!(
        stdout.contains("96 cases: 96 match, 0 diverged, 0 drift, 0 stale entries"),
...
fn default_mode_gates_help_cases_exit_0() {
    ...
    // (diverged stays 17).
    // NRN-384 ports the MCP catalog over the owner session and gates the `mcp`
    // tools/call cases (get-alpha flipped ported + get-missing→isError,
    // count-by-type, validate-code), all byte-exact matches against the oracle (no
    // ledger entry): the gated total grows to 95 and the match count to 78
    // (diverged stays 17). The lone tools/list case stays `ported: false`
    // (audit-tool absence + intentional schema divergences make the full catalog
    // un-byte-matchable — see `MCP_CASES`), so the gated `ported` filter skips it.
    assert!(
        stdout.contains("95 cases: 78 match, 17 diverged, 0 drift, 0 stale entries"),
